...
function AcademicsPage() {
  ...
  return (
    <Page>
      ...
      <div className="mt-8">
        <h2 className="mb-3 text-lg font-semibold tracking-tight">Report cards</h2>
        {cardsQ.isLoading ? (
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {cards.map((c) => {
              const cardGrade = c.overall_grade ?? gradeFor(c.percentage).letter;
              return (
                <SectionCard
                  key={c.id}
                  title={c.term}
                  ...
                  actions={<StatusPill status={c.file_url ? "submitted" : "pending"}>{cardGrade}</StatusPill>}
                >
